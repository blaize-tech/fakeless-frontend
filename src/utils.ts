...
export async function initContract() {
  ...
  const nearConfigDtoken = getConfig(
    process.env.NODE_ENV || 'testnet',
    'dev-1639068270320-45550015151191',
  );
  const nearConfigController = getConfig(
    process.env.NODE_ENV || 'testnet',
    'dev-1639146095042-48833853079859',
  );
  ...
  const controllerContract = new nearAPI.Contract(
    ...
    {
      // View methods are read-only – they don't modify the state, but usually return some value
      viewMethods: ['has_collaterall', 'get_interest_rate', 'borrow_allowed', 'supply_allowed'],
      changeMethods: ['set_borrow_cap', 'set_interest_rate_model', 'add_market', 'add_market_'],
    },
  );
